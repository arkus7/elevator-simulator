...
export class BuildingConfig {
  @ApiProperty({
    description: 'The minimum floor of the building',
    example: -2,
  })
  minFloor: number;
  ...
  maxFloor: number;
  @ApiProperty({
    description: 'The number of elevators in the building',
    example: 3,
  })
...
export class ElevatorState {
  ...
  @ApiProperty({
    ...
    example: ElevatorDirection.Up,
  })
  direction: ElevatorDirection;
  @ApiProperty({
    description: 'The state of the elevator doors',
    enum: ElevatorDoorState,
    example: ElevatorDoorState.Open,
  })
  doorState: ElevatorDoorState;
  @ApiProperty({
    description: 'The motion state of the elevator',
    enum: ElevatorMotionState,
    example: ElevatorMotionState.Moving,
  })
  motionState: ElevatorMotionState;
  @ApiProperty({
    description: 'The destination floors of the elevator',
    example: [1, 3, 5],
  })
  destinationFloors: number[];
  @ApiProperty({
    description: 'The status of the elevator',
    enum: ElevatorStatus,
    example: ElevatorStatus.Active,
  })
